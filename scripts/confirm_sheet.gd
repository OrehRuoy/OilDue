...
static func present(host: Node, title: String, body: String, safe_text: String, danger_text: String, on_danger: Callable) -> void:
	if host == null:
		return
	...
	sheet.setup(title, body, safe_text, danger_text, on_danger)


func setup(title: String, body: String, safe_text: String, danger_text: String, on_danger: Callable) -> void:
	...
	%DangerButton.text = danger_text if danger_text.strip_edges() != "" else "Delete"
	_pad_home_indicator()
